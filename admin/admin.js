...
const FIELDS=[
  ...
  {key:'rfi',label:'RFI',aliases:['rfi']},
  {key:'msp',label:'MSP',aliases:['msp']},
  {key:'efi',label:'EFI',aliases:['efi']},
  {key:'gefi',label:'gEFI',aliases:['gefi','g efi']},
...
function normalizeRows(){
  ...
  state.rows=state.rawRows.map((row,index)=>{
    ...
    const normalized={
      ...
      rfi:cleanNumber(valueAt(row,'rfi')),msp:cleanNumber(valueAt(row,'msp')),efi:cleanNumber(valueAt(row,'efi')),gefi:cleanNumber(valueAt(row,'gefi')),
...
function renderCatalogRows(){
  ...
  $('#existingRows').innerHTML=rows.length?rows.map(row=>`<tr><td>${escapeHtml(row.codigo)}</td><td>${escapeHtml(row.nombre)}</td><td>${row.nm??'—'}</td><td>${escapeHtml(row.beta||'—')}</td><td>${escapeHtml(row.kappa||'—')}</td><td><span class="status-pill ${row.activo?'on':'off'}">${row.activo?'Activo':'Inactivo'}</span></td><td>${row.updated_at?new Date(row.updated_at).toLocaleDateString('es-MX'):'—'}</td><td class="row-actions"><a class="text-btn" href="editor.html?codigo=${encodeURIComponent(row.codigo)}">Editar</a>${row.activo?`<button class="text-btn" data-copy-link="${escapeHtml(row.codigo)}">Copiar link</button><button class="text-btn danger" data-deactivate="${escapeHtml(row.codigo)}">Eliminar</button>`:`<button class="text-btn" data-restore="${escapeHtml(row.codigo)}">Restaurar</button><button class="text-btn danger" data-purge="${escapeHtml(row.codigo)}">Borrar definitivo</button>`}</td></tr>`).join(''):'<tr><td colspan="8" class="empty">Sin resultados.</td></tr>';
}
...
$('#existingRows').addEventListener('click',async event=>{
  const deactivate=event.target.closest('[data-deactivate]'),restore=event.target.closest('[data-restore]'),purge=event.target.closest('[data-purge]'),copyLink=event.target.closest('[data-copy-link]');
  if(copyLink){
    const url=`https://geneticauniversal.com/toro/${encodeURIComponent(copyLink.dataset.copyLink)}`;
    try{await navigator.clipboard.writeText(url);const original=copyLink.textContent;copyLink.textContent='¡Copiado!';setTimeout(()=>{copyLink.textContent=original;},2000);}
...
$('#downloadTemplate').addEventListener('click',()=>{
  ...
  const exampleByKey={
    ...
    nm:955,cm:992,cfp:85,feedSaved:104,rfi:24,msp:1,efi:11.5,gefi:13,
