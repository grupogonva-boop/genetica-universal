...
async function loadCatalog(){try{const data=await api('/api/sires');const rows=data.sires||[];$('#totalSires').textContent=rows.length;$('#activeSires').textContent=rows.filter(row=>row.activo).length;$('#a2Sires').textContent=rows.filter(row=>row.beta==='A2/A2').length;$('#lastImport').textContent=data.lastImport?new Date(data.lastImport).toLocaleDateString('es-MX'):'—';$('#existingRows').innerHTML=rows.length?rows.slice(0,30).map(row=>`<tr><td>${escapeHtml(row.codigo)}</td><td>${escapeHtml(row.nombre)}</td><td>${row.nm??'—'}</td><td>${escapeHtml(row.beta||'—')}</td><td>${escapeHtml(row.kappa||'—')}</td><td>${new Date(row.updated_at).toLocaleDateString('es-MX')}</td></tr>`).join(''):'<tr><td colspan="6" class="empty">Sin datos.</td></tr>';}catch(error){$('#existingRows').innerHTML=`<tr><td colspan="6" class="empty">${escapeHtml(error.message)}</td></tr>`;}}
