...
let activeF="all",searchTerm="",sortKey="nm",sortDir=-1,columnFilters={},fichaStep=0;
...
function renderTable(){
  ...
  tbody.innerHTML=list.map(t=>`<tr data-i="${TOROS.indexOf(t)}" tabindex="0" aria-label="Abrir ficha 360 de ${t.nombre}">
    <td><div class="sire-id"><img class="sire-thumb" src="${t.foto}" alt="" loading="lazy"><div><b>${t.nombre}</b><small>Holstein · Genómico</small></div></div></td>
  ...
  tbody.querySelectorAll('tr[data-i]').forEach(row=>{const open=()=>openModal(TOROS[row.dataset.i]);row.addEventListener('click',open);row.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();open();}});});
}
...
function handleFichaClick(e){const step=e.target.closest('[data-step]');if(step){setFichaStep(Number(step.dataset.step));return;}if(e.target.closest('.wizard-back'))setFichaStep(fichaStep-1);if(e.target.closest('.wizard-next')){if(fichaStep<2)setFichaStep(fichaStep+1);else{const t=TOROS[Number(document.getElementById('fichaContent').dataset.toro)];window.open(waLink(CONFIG.msgToro(t)),'_blank','noopener');}}}
function closeModal(){document.getElementById('modalBack').classList.remove('open');document.body.style.overflow='';}
